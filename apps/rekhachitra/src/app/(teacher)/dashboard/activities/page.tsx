...
import { listMine, deleteActivity, setStatus, saveActivity } from "@/lib/data/activities";
...
export default function ActivitiesPage() {
  ...
  // Mint a share slug if needed, then copy the public /a/<slug> link.
  const handleShare = async (activity: Activity): Promise<void> => {
    const saved = activity.shareSlug ? activity : await saveActivity(activity);
    if (saved.shareSlug !== activity.shareSlug) {
      setActivities((prev) => prev.map((a) => (a.id === saved.id ? saved : a)));
    }
    try {
      await navigator.clipboard.writeText(`${window.location.origin}/a/${saved.shareSlug}`);
    } catch {
    ...
    <div className="p-8 max-w-5xl">
      ...
      {loaded && activities.length > 0 && (
        <p className="mt-6 text-xs text-center" style={{ color: "var(--color-subtle)" }}>
          Activities are saved to the cloud and shareable by link — hit Share to copy a public link any teacher can open or copy.
        </p>
      )}
    </div>
  );
