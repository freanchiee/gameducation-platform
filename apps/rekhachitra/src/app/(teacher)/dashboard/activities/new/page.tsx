...
import {
  ...
  Eye, Type, Image as ImageIcon, MessageSquare, BarChart2,
...
import { getById, saveActivity } from "@/lib/data/activities";
...
function NewActivityPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      // Load a specific activity by ?id= (from list "Edit" / a forked copy) — durable store
      if (idParam) {
        const found = await getById(idParam);
        if (cancelled) return;
        setActivity(found ? migrateActivity(found) : SEED_ACTIVITY);
        return;
      }
      // Otherwise restore the last local autosave draft, else seed
      const saved = localStorage.getItem(LS_AUTOSAVE_KEY);
      if (saved) {
        try {
          if (!cancelled) setActivity(migrateActivity(JSON.parse(saved) as Activity));
          return;
        } catch {
          /* fall through */
        }
      }
      if (!cancelled) setActivity(SEED_ACTIVITY);
    })();
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    autosaveTimer.current = setTimeout(() => {
      try {
        ...
        // Keep a local draft for crash recovery…
        localStorage.setItem(LS_AUTOSAVE_KEY, JSON.stringify(stamped));
        // …and persist to the durable store (Supabase + mirror).
