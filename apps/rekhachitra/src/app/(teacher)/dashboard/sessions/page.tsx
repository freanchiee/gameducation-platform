...
import { getById } from "@/lib/data/activities";
...
function SessionsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Pull the activity from the durable store so a shared/forked activity
      // (not just one in this browser) can be launched.
      let activityToStart = DEMO_ACTIVITY;
      if (activityId) {
        const found = await getById(activityId);
    ...
    return () => {
      cancelled = true;
      reset(); // clear session on unmount / between Strict Mode runs
    };
...
function LiveRoster({ joinCode, totalSlides }: { joinCode: string; totalSlides: number }) {
  ...
  useEffect(() => {
    // Defer the first fetch so the setState inside refresh() runs after commit,
