...
import DesmosCalculator, { type DesmosHandle } from "@/desmos/components/graph/DesmosCalculator";
import { Button } from "@/desmos/components/ui/Button";
import { Badge } from "@/desmos/components/ui/Badge";
import { useBuilderStore } from "@/desmos/lib/store/session.store";
import { generateId } from "@/desmos/lib/utils/session";
import { getActivity, saveActivity } from "@/desmos/lib/activities-store";
import type { Activity, Slide, CheckpointType, ContentBlock } from "@/desmos/types";
import { cn } from "@/desmos/lib/utils/cn";
...
function NewActivityPage() {
  ...
  useEffect(() => {
    ...
    // getActivity checks Supabase first (cross-device), then localStorage.
    const idParam = searchParams.get("id");
    if (idParam) {
      void (async () => {
        const found = await getActivity(idParam);
        setActivity(migrateActivity(found ?? SEED_ACTIVITY));
      })();
      return;
    }
  ...
  useEffect(() => {
    ...
    autosaveTimer.current = setTimeout(() => {
      try {
        ...
        // Persist to Supabase (also mirrors to localStorage)
        void saveActivity(stamped);
