...
import { JoinCodeDisplay } from "@/desmos/components/session/JoinCodeDisplay";
import DesmosCalculator from "@/desmos/components/graph/DesmosCalculator";
import { useTeacherSessionStore } from "@/desmos/lib/store/session.store";
import { LS_LIVE_SESSION_PREFIX, LS_STUDENT_PREFIX } from "@/desmos/lib/store/session.store";
import type { Activity, Slide, ContentBlock } from "@/desmos/types";
import { Badge } from "@/desmos/components/ui/Badge";
import { Avatar } from "@/desmos/components/ui/Avatar";
import { getActivity } from "@/desmos/lib/activities-store";
...
function SessionsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (activityId) {
        const found = await getActivity(activityId);
        if (found) activityToStart = found;
      }
      if (cancelled) return;
    ...
    return () => { cancelled = true; reset(); }; // clear session on unmount / between Strict Mode runs
