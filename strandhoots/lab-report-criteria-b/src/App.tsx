...
// A self-contained virtual magnetism lab so students gather real data (and a
// graph) to ground their research question, hypothesis, variables and method.
const LabSimulation = () => {
  const [mode, setMode] = useState<'distance' | 'magnets'>('distance');
  const [distance, setDistance] = useState(6); // cm
  const [magnets, setMagnets] = useState(1);
  const [data, setData] = useState<{ iv: number; dv: number }[]>([]);

  const reading = mode === 'distance' ? Math.round(1200 / (distance * distance)) : Math.round(60 * magnets);
  const iv = mode === 'distance' ? distance : magnets;
  const ivLabel = mode === 'distance' ? 'Distance (cm)' : 'Number of magnets';

  const record = () => {
    const noisy = Math.max(1, Math.round(reading * (1 + (Math.random() - 0.5) * 0.06)));
    setData((prev) => [...prev, { iv, dv: noisy }].slice(-12));
  };

  const W = 320, H = 180, pad = 30;
  const maxX = Math.max(mode === 'distance' ? 20 : 6, ...data.map((d) => d.iv), 1);
  const maxY = Math.max(10, ...data.map((d) => d.dv));
  const px = (x: number) => pad + (x / maxX) * (W - pad - 10);
  const py = (y: number) => H - pad - (y / maxY) * (H - pad - 10);
  const sorted = [...data].sort((a, b) => a.iv - b.iv);

  return (
    <div className="space-y-5">
      <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-800">
        🔬 Use this virtual bench to <strong>collect real data</strong>, then use your results to write your research
        question, hypothesis, variables and method in the other tabs.
      </div>

      <div className="inline-flex overflow-hidden rounded-lg border">
        {([['distance', '📏 Effect of Distance'], ['magnets', '🧲 Number of Magnets']] as const).map(([m, label]) => (
          <button
            key={m}
            onClick={() => setMode(m)}
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="rounded-xl border p-4">
          <svg viewBox="0 0 320 150" className="w-full">
            ...
            <line x1="20" y1="120" x2="300" y2="120" stroke="#94a3b8" strokeWidth="2" />
            <rect x="270" y="70" width="30" height="50" rx="3" fill="#1e293b" />
            <text x="285" y="62" textAnchor="middle" fontSize="9" fill="#475569">sensor</text>
            {mode === 'distance' ? (
              <g transform={`translate(${30 + (1 - distance / 20) * 180},0)`}>
                <rect x="0" y="78" width="36" height="34" rx="3" fill="#ef4444" />
                <rect x="0" y="78" width="18" height="34" fill="#3b82f6" />
                <text x="18" y="128" textAnchor="middle" fontSize="9" fill="#475569">{distance} cm</text>
              </g>
            ) : (
              <g transform="translate(205,0)">
                {Array.from({ length: magnets }).map((_, i) => (
                  <rect key={i} x={i * 6} y="78" width="34" height="34" rx="3" fill={i % 2 ? '#f97316' : '#ef4444'} opacity="0.9" />
                ))}
                <text x="20" y="128" textAnchor="middle" fontSize="9" fill="#475569">{magnets} magnet{magnets > 1 ? 's' : ''}</text>
              </g>
            )}
            {[18, 30, 42].map((r, i) => (
              <circle key={r} cx="285" cy="95" r={r} fill="none" stroke="#3b82f6" strokeWidth="2" opacity={Math.min(0.5, reading / 600) * (1 - i * 0.25)} />
            ))}
          </svg>

          {mode === 'distance' ? (
            <div className="mt-3">
              <label className="text-sm text-gray-600">Distance: <strong>{distance} cm</strong></label>
              <input type="range" min={2} max={20} value={distance} onChange={(e) => setDistance(Number(e.target.value))} className="w-full" />
            </div>
          ) : (
            <div className="mt-3 flex items-center gap-3">
              <span className="text-sm text-gray-600">Magnets:</span>
              <button onClick={() => setMagnets((m) => Math.max(1, m - 1))} className="size-8 rounded bg-gray-200 font-bold">−</button>
              <strong className="w-6 text-center">{magnets}</strong>
              <button onClick={() => setMagnets((m) => Math.min(6, m + 1))} className="size-8 rounded bg-gray-200 font-bold">+</button>
            </div>
          )}

          <div className="mt-4 flex items-center justify-between rounded-lg bg-slate-900 px-4 py-3 text-white">
            <span className="text-xs uppercase tracking-wide text-slate-400">Gaussmeter</span>
            <span className="font-mono text-2xl font-bold text-green-400">{reading}<span className="ml-1 text-sm">mT</span></span>
          </div>

          <div className="mt-3 flex gap-2">
            <button onClick={record} className="flex-1 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">➕ Record reading</button>
            <button onClick={() => setData([])} className="rounded-lg border px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Reset</button>
          </div>
        </div>

        <div className="rounded-xl border p-4">
          <h4 className="mb-2 font-semibold text-gray-800">Your data ({data.length})</h4>
          {data.length === 0 ? (
            <p className="text-sm text-gray-500">Record a few readings to build your data table and graph.</p>
          ) : (
            <>
              ...
              <svg viewBox={`0 0 ${W} ${H}`} className="mt-3 w-full">
                <line x1={pad} y1={H - pad} x2={W - 10} y2={H - pad} stroke="#cbd5e1" />
                <line x1={pad} y1={10} x2={pad} y2={H - pad} stroke="#cbd5e1" />
                <text x={(W + pad) / 2} y={H - 6} textAnchor="middle" fontSize="9" fill="#64748b">{ivLabel}</text>
                <text x="8" y="12" fontSize="9" fill="#64748b">mT</text>
                <polyline fill="none" stroke="#93c5fd" strokeWidth="1.5" points={sorted.map((d) => `${px(d.iv)},${py(d.dv)}`).join(' ')} />
                {data.map((d, i) => (
                  <circle key={i} cx={px(d.iv)} cy={py(d.dv)} r="3.5" fill="#2563eb" />
                ))}
              </svg>
            </>
          )}
        ...
        <div className="mb-6">
          <div className="flex border-b">
            ...
            <button
              ...
              className={`py-2 px-4 font-medium ${currentTab === 'sim' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-600'}`}
            >
              🔬 Simulation
            </button>
          </div>
